...
switch ($_GET['action']) {
	...
	//form to edit license record
		...
		<tr>
		<td colspan='2' id='license-form-title'><span class='headerText'>License</span><br /></td>
		</tr>

		<tr>
		<td colspan='2' class='below-title'><label for="shortName" class="formText">License Name:</label>  <span id='span_error_licenseShortName' class='errorText'></span><br /><textarea name='licenseShortName' id = 'licenseShortName' cols='38' rows='2'><?php echo $license->shortName; ?></textarea></td>
		</tr>
		...
		<tr>
		<td colspan='2'><label for="consortiumID" class="formText">Consortium:</label><br />
		...
		?>
		<br />
		<span id='span_newConsortium'><a href="javascript:newConsortium();">add consortium</a></span>
		<?php } ?>
		...
		<td style="padding-top:8px;float:left"><input type='button' value='submit' name='submitLicense' id ='submitLicense'></td>
		<td style="padding-top:8px;padding-right:8px;float:left;margin-left:6px;"><input type='button' value='cancel' onclick="tb_remove()"></td>
		</tr>
		</table>
		...
		<?php
			...
			foreach ($document->getSignaturesForDisplay() as $display) {
				...
				if ($signatureID == $display['signatureID']){
					...
					echo "</td>";
					echo "<td><a href='javascript:void(0)' id='commitUpdate' name='commitUpdate'>commit update</a></td>";
	...
	//form to add/edit expressions
		...
		<span id='span_newExpressionType'><a href="javascript:newExpressionType();">add expression type</a></span>
		...
		<td colspan='2'><label for="qualifierID" class="formText">Qualifier:</label><br />
		...
		<tr>
		<td colspan='2'><label for="documentText" class="formText">Document Text:</label><br /><textarea name='documentText' id = 'documentText' cols='48' rows='10'><?php echo $expression->documentText; ?></textarea></td>
		</tr>
	...
	//form to add / edit expression notes (internal and display notes)
		...
		<th><b><?php echo ucfirst($noteType); ?> Notes</b></th>
		<th>&nbsp;</th>
		<th>&nbsp;</th>
		</tr>
		...
		<?php
			...
			if ($expressionNoteID == ""){
				...
				echo "<td><a href='javascript:addExpressionNote();'>add</a></td>";
				echo "<td>&nbsp;</td>";
				echo "</tr>";
			}
		?>
	...
	case 'getAdminUpdateForm':
		...
		<table class="thickboxTable" style="width:200px;">
		<tr>
		<td colspan='3'><span class='headerText'>Update</span><br /><span id='span_errors' style='color:#F00;'></span><br /></td>
		</tr>
	...
	case 'getExpressionTypeForm':
		...
		<option value='Display' <?php if ((isset($expressionType)) && ($expressionType->noteType == 'Display')) echo "selected"; ?>>Display</option>
		</select>
		</td>
		</tr>

		<tr><td colspan='2'><span class='smallText'>* Note type of display allows for terms tool use</span></td></tr>


		<tr>
		<td style="padding-top:18px;"><input type='button' value='<?php echo $update; ?>' onclick='javascript:window.parent.submitExpressionType();' id='update-expression-type'></td>
		<td style="padding-top:18px;padding-right:8px;text-align:right;"><input type='button' value='cancel' onclick="window.parent.tb_remove(); return false" id='cancel-expression-type'></td>
		</tr>
		</table>
		</div>
